  ...
  return (
    <section className="scenarios" id="cenarios" aria-labelledby="scenarios-title">
      <div className="scenarios-heading">
        <div><p className="section-index">03 — 07</p><p className="section-kicker">Cenários em rotação</p></div>
        <h2 id="scenarios-title">Explore o que<br />já existiu aqui.</h2>
        <p className="scenarios-intro">Cada ambiente nasce com prazo para desaparecer. Arraste, descubra e imagine o próximo.</p>
      </div>
      <div className="gallery-shell">
        <div ref={trackRef} className="gallery-track" onScroll={syncCurrent} tabIndex={0} aria-label="Galeria horizontal de sete cenários">
          {scenes.map((scene, index) => (
            <article className="scene" key={scene.title} aria-label={`Cenário ${index + 1} de 7: ${scene.title}`}>
